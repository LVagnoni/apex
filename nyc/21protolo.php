...
<title>apexart :: NYC Exhibition Program 2021-22 :: Submissions</title>
<body bgcolor="#000000">

<table  width="845" align="center" border="0">
  <tr>
    <td><div class="exhibitionProgram">NYC Exhibition Program 2021-22 Submissions</div>
    <?php
	...
	$query="SELECT * FROM proposal WHERE is_accepted=0 ORDER BY id desc";//WHERE is_accepted=0
	...
	while ($row = mysql_fetch_assoc($result)) {
		...
		$ff=$row["id"];
		$query2="SELECT avg(score) as 'aver' FROM rating Where proposal_id=$ff";
		$result2 = mysql_query($query2, $conn); 
		...
		$query2="SELECT count(*) as 'cnt' FROM rating Where proposal_id=$ff";
		$result2 = mysql_query($query2, $conn); 
		...
		echo "<td class='grayback' width='50' align=left valign=top class='standardTextLg';'>#".$row["id"]."</td>";
		$SESSION['id'] = $row["id"];
		...
		echo "<td class='grayback' align=left valign=top class='standardTextLg'>Submitted from: ".$row["city"].", ".$row["state"]." - ".$row["country"]."</td>";
		echo "<td class='grayback' align=left valign=top class='standardTextLg' style='text-transform: uppercase;'></td>";
	    echo "</tr>";
	    echo "</td></tr>";
        ...
        <a href='mailto:".$row["email"]."?subject=Re:%20Your%20apexart%20Open%20Call%20Proposal%20#".$row["id"]."&body=Dear%20".$row["submitters"].",%0A%0AThank%20you%20for%20submitting%20a%20proposal%20to%20apexart&#39;s%20Open%20Call%20for%20exhibitions.%20We%20are%20writing%20to%20get%20clarification%20on%20your proposal,%20as%20it%20currently%20does%20not%20seem%20to%20meet%20our%20qualifications%20and%20will%20not%20be%20sent%20to%20the%20jury%20without%20additional%20information.%0A%0AIt%20appears%20your%20proposal:%0A%0A

					- is a solo exhibition, or does not include individual works by at least three artists working independently (and not as a collective). Proposals must be for group exhibitions consisting of at least 3 artists working independently.%0A
					...
					- is intended to take place somewhere other than 291 Church St in NYC.%0A
...
Email Submitter&nbsp;&nbsp;</a>
	  	...
	  	echo "<td  class='grayback' colspan=2 align='right'>Disqualify Submission #".$row["id"]."";
