<?php
	...
	if (isset($_SESSION["userID"]) && isset($_GET["vote"])){
		...
		$query="INSERT INTO guo_testrating (juror_id, proposal_id, score, created_on) VALUES ";
		$leng=strlen($query);
		$i=0;
		...
		foreach($v as $pr=>$sc){
            ...
            $sql = "SELECT * FROM guo_testproposal WHERE checked_count < vote_limiter AND id = $pr";
            ...
            if ($count > 0) {
                ...
                $sql = "UPDATE guo_testproposal SET checked_count = checked_count + 1 WHERE id = $pr";
                if(mysql_query($sql)) {
                    $query2="SELECT * FROM guo_testrating where juror_id=$x AND proposal_id=$pr";
	...
	else header("location: jurypage.php"); 
...
<tr><td class="standardText" align="center"> 	    
	    ...
	    <br><a href="jurypage.php">Back to submissions list</a>
