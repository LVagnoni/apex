<?php
	...
	if (isset($_SESSION["userID"]) && isset($_GET["vote"])){
		...
		$query="INSERT INTO intl22_rating (juror_id, proposal_id, score, created_on) VALUES ";
		$leng=strlen($query);
		$i=0;
		...
		foreach($v as $pr=>$sc){
            ...
            $sql = "SELECT * FROM intl22_proposal WHERE checked_count < vote_limiter AND id = $pr";
            ...
            if ($count > 0) {
                ...
                $sql = "UPDATE intl22_proposal SET checked_count = checked_count + 1 WHERE id = $pr";
                if(mysql_query($sql)) {
                    $query2="SELECT * FROM intl22_rating where juror_id=$x AND proposal_id=$pr";
