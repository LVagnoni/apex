...
session_start();
    ...
    if (isset($_POST["username"])){
        ...
        $query="SELECT * FROM intl22_juror WHERE username='".$xx."' LIMIT 1";
        ...
        if (is_array($row) AND !empty($row)) 
        {
            ...
            $query3="UPDATE intl22_juror SET login_on = NOW() WHERE `id`=".(int)$row["id"]." LIMIT 1";
            //UPDATE Persons SET Age = '36' WHERE FirstName = 'Peter'
            $result3 = mysql_query($query3, $conn);

            header("location: jurypage-test.php");
            exit();
    ...
    if ($auth_failed) :
        echo "<tr align='center'><td class='verdana' size='4'>";
            echo "<br><br><br><br><br><br><br><center><a href='login-test.php'>WRONG LOGIN<br>Try Again</a><br><br><br><br><br><br><br>";
        echo "</tr></td>";
    endif;
        ?>
...
<br><br><br>
  ...
  <strong>Q: Do I judge the writing or the idea?</strong><br />
          A: Mainly, you should judge how well the idea comes through. The substance of the idea is important as is the ability to convince you of its value. Please note that many proposals come from non-native English speakers, so while the grammar may not be perfect, the strength of the idea should shine through in a good proposal. <strong>Artists, venues, and dates are subject to availability and are not necessarily confirmed.</strong><br />
  <br />
...
<strong>Q: When is the voting deadline?</strong><br />
    A: April 4, 2022, 23:59 PST </p>
    <br />
                

                <form action="login-test.php" method="post">username:
                    <input type="text" size="20" maxlength="30" name="username" style="background-color:#000000;">password:
                    <input type="text" size="20" maxlength="30" name="password" style="background-color:#000000;">
                    <br><br \>
