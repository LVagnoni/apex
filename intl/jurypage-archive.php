...
<!--<title>apexart :: Open Call Program :: Juror Voting</title>-->
            ...
            $sql = "SELECT * FROM guo_testproposal WHERE checked_count < vote_limiter";
            $result = mysql_query($sql, $conn);
            $testCount = mysql_num_rows($result);


            $sql = "SELECT MIN(checked_count) as checked_count, MAX(vote_limiter) as vote_limiter, MIN(vote_limiter) as min_vote_limiter, COUNT(*) as total FROM guo_testproposal";
            $result = mysql_query($sql, $conn);
            ...
            if ($result) {
                ...
                if($maxVoteLimiter === 0) {
                    ...
                    $sql = "UPDATE guo_testproposal SET vote_limiter = " . VOTE_LIMITER_INCREASE_COUNT;
                    $res = mysql_query($sql, $conn);
                }
                else if ($minVoteLimiter < $maxVoteLimiter) {
                    // update vote limiter to max value
                    $sql = "UPDATE guo_testproposal SET vote_limiter = " . $maxVoteLimiter;
                    $res = mysql_query($sql, $conn);
                    ...
                    $sql = "UPDATE guo_testproposal SET vote_limiter = " . $updateVoteLimiter;
            ...
            $query2="SELECT * FROM guo_testproposal WHERE is_accepted!=1"; // WHERE is_accepted=1 ";
            $result2 = mysql_query($query2, $conn);
            $nums=mysql_num_rows($result2);

            $query2="SELECT * FROM guo_testrating WHERE juror_id=".$_SESSION["userID"];
            $result2 = mysql_query($query2, $conn);
                         ...
                         <table width="950" border="0" align="center" bgcolor="#FFFFFF">
                             <tr valign="top">
                                 <td>
                                     ...
                                     $sql = "SELECT proposal_id FROM guo_testrating WHERE juror_id = " . $_SESSION["userID"];
                                     $result = mysql_query($sql, $conn);
                                     $condition = "";
                                     ...
                                     $query="SELECT * FROM guo_testproposal";
